...
function legendClickCallback(s) {
	for (var a = (s = s || window.event).target || s.srcElement; 'LI' !== a.nodeName; ) a = a.parentElement;
	...
	i.legend.options.onClick.call(i, s, i.legend.legendItems[n]),
		i.isDatasetVisible(n) ? a.classList.remove('hidden') : a.classList.add('hidden');
}
function readURL(s) {
	if (s.files && s.files[0]) {
		var a = new FileReader();
		(a.onload = function(a) {
			$(s).parent('.input-file-image').find('.img-upload-preview').attr('src', a.target.result);
		}),
...
function changeContainer() {
	1 == showSignIn ? containerSignIn.css('display', 'block') : containerSignIn.css('display', 'none'),
		1 == showSignUp ? containerSignUp.css('display', 'block') : containerSignUp.css('display', 'none');
}
	...
	$(document).ready(function() {
		...
		if (!c) {
			(C = $('.sidenav-toggler')).on('click', function() {
				1 == u
					? ($('html').removeClass('nav_open'), C.removeClass('toggled'), (u = 0))
		...
		if (!d) {
			...
			k.on('click', function() {
				1 == m
					? ($('html').removeClass('topbar_open'), k.removeClass('toggled'), (m = 0))
		...
		if (!g) {
			...
			$('.wrapper').hasClass('sidebar_minimize') &&
				((v = 1), w.addClass('toggled'), w.html('<i class="icon-options-vertical"></i>')),
				w.on('click', function() {
					1 == v
